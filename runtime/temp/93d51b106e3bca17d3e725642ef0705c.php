...
<div class="ban_box">
	<div class="banner" >
		<ul class="full-screen-slides" id="fullScreenSlides">
			<?php $service = new data\service\Platform;$list = $service->getPlatformAdvPositionDetail("1103", "*");$list = json_encode($list);$list = json_decode($list, ture); if(is_array($list['adv_list']) || $list['adv_list'] instanceof \think\Collection || $list['adv_list'] instanceof \think\Paginator): if( count($list['adv_list'])==0 ) : echo "" ;else: foreach($list['adv_list'] as $key=>$vo): if($vo['adv_image'] != ''): if($k == 0): ?>
	...
	<div class="maind">
		<?php $service = new data\service\Platform;$list = $service->getPlatformAdvPositionDetail("1103", "*");$list = json_encode($list);$list = json_decode($list, ture); if($list['adv_list'][0]['adv_image'] == ''): ?>
