<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:44:"template/wap\default_new\Login\register.html";i:1536465243;s:38:"template/wap\default_new\urlModel.html";i:1510824803;}*/ ?>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
var STATIC = "__STATIC__";
	...
	function register_member(){
		...
		if(cfpassword == ''){
			showBox("<?php echo lang('confirm_password_can_not_be_empty'); ?>", "error");
			return false;
		}
		if(password != cfpassword){
			showBox("<?php echo lang('two_password_input_is_inconsistent'); ?>", "error");
			$("#cfpassword").focus();
			return false;
		}
		...
		$.ajax({
			...
			success : function(data) {
				if(data["code"] > 0 ){
					showBox(data["message"], "success", "<?php echo __URL('APP_MAIN/member/index'); ?>");
				}else{
					is_member_sub = false;
	...
	function register_mobile(){
		...
		if(mobile == ''){
			...
			showBox("<?php echo lang('mobile_phone_number_is_wrong'); ?>", "error");  
	        return false; 
	    } else if(mobile_is_has == 0){
	    	showBox("<?php echo lang('mobile_phone_is_registered'); ?>", "error");
			return false;
	    } 
		if(vertification == ''){
			showBox("<?php echo lang('verification_code_cannot_be_null'); ?>", "error");
		...
		if(verify_code == ''){
			showBox("<?php echo lang('mobile_phone_dynamic_password_can_not_be_empty'); ?>", "error");
			return false;
		}
		...
		$.ajax({
 		    ...
 		    success:function(data){
 		    	 if (data['code'] == 0) { 
 					...
 					is_mobile_sub = false;
 					showBox("<?php echo lang('mobile_phone_dynamic_code_error'); ?>", "error");
 					return false;
 				}
 		    }
 		}); 
...
function verifyUsername(username){
	...
	if(/.*[\u4e00-\u9fa5]+.*$/.test(username) ){
		is_true = 1;
		showBox("<?php echo lang('user_name_cannot_contain_chinese_characters'); ?>", "error");
		return is_true;
	}
	if(/\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*/.test(username) ){
		is_true = 1;
		showBox("<?php echo lang('user_name_canno_be_mailbox'); ?>", "error");
		return is_true;
	}
	if(/^1(3|4|5|7|8)\d{9}$/.test(username) ){
		is_true = 1;
		showBox("<?php echo lang('user_name_canno_be_phone'); ?>", "error");
		return is_true;
	}
	...
	$.each(usernme_verify_array,function(k,v){
		if($.trim(v) != ""){
			if(username.indexOf(v) >= 0){
				is_true = 1;
				showBox("<?php echo lang('username_cannot_includ'); ?>"+v+"<?php echo lang('such_characters'); ?>", "error");
				return false;
			}
		}
	});
...
function verifyPassword(password){
	...
	if(min_length  > 0){
		if(password.length < min_length){
			is_true = 1;
			showBox("<?php echo lang('minimum_password_length'); ?>"+min_length, "error");						
			return is_true;
		}
	...
	if(/.*[\u4e00-\u9fa5]+.*$/.test(password) ){
		is_true = 1;
		showBox("<?php echo lang('password_cannot_includ_chinese_characters'); ?>", "error");
		return is_true;
	}
	...
	if($.trim(regex_str) != "" && regex_str != undefined){
		...
		if(regex_str.indexOf("number") >= 0){
			...
			if(!number_test.test(password)){
				is_true = 1;
				showBox("<?php echo lang('password_must_contain_numbers'); ?>", "error");									
				return is_true;
			}
		}
		//验证是否包含小写字母
		if(regex_str.indexOf("letter") >= 0){
			...
			if(!letter_test.test(password)){
				is_true = 1;
				showBox("<?php echo lang('password_must_have_lowercase_letters'); ?>", "error");								
				return is_true;
			}
		}
		//验证是否包含大写字母
		if(regex_str.indexOf("upper_case") >= 0){
			...
			if(!upper_case_test.test(password)){
				is_true = 1;
				showBox("<?php echo lang('password_must_have_uppercase_letters'); ?>", "error");					
				return is_true;
			}
		}
		//验证是否包含特殊字符
		if(regex_str.indexOf("symbol") >= 0){
			...
			if(!symbol_test.test(password)){
				is_true = 1;
				showBox("<?php echo lang('password_must_contain_symbols'); ?>", "error");						
				return is_true;
			}
		} 
	}
		...
		<!-- <form id="regForm" action="APP_MAIN/login/register" method="post" onsubmit="return check()"> -->
			...
			<div id="nk_text1" style="display:block;">
				...
				<div class="nk-cont" >
						...
						<input type="text" name="captcha" id="register-captcha"  placeholder="<?php echo lang('please_enter_verification_code'); ?>">
